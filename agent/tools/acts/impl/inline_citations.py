...
class InlineCitationsAct(IToolAct[InlineCitationsToolCall]):
    ...
    async def act(
        self,
        tool_call: InlineCitationsToolCall,
    ) -> ToolActResult:
        with tool_span(
            tracer,
            "InlineCitationsAct.act",
            tool_call,
        ) as span:
            yield (
                f"Validating {len(tool_call.params.citations)} inline citation(s)\n\n"
            )
            ...
            for item in tool_call.params.citations:
                ...
                for snippet in item.snippets:
                    ...
                    if not quote:
                        continue
                    if quote in chunk_text or find_quote_span(
                        chunk_text,
                        quote,
                    ):
